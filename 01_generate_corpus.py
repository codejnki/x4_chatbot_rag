...
def summarize_content_in_batches(content: str) -> str:
    ...
    if content_tokens < MAX_CONTEXT_TOKENS:
        try:
            # Only try to summarize if there's a Markdown table present
            if "|" not in content and "---" not in content:
                return content
                
            formatted_prompt = SUMMARIZER_PROMPT_TEMPLATE.format(content=content)
            response = CLIENT.chat.completions.create(
                model="local-model", messages=[{"role": "user", "content": formatted_prompt}], temperature=0.2,
            )
            summary = response.choices[0].message.content
            # Combine the summary with the original content
            return f"{summary}\n\n{content}"
    ...
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=MAX_CONTEXT_TOKENS, chunk_overlap=200, length_function=lambda text: len(tokenizer.encode(text)),
    )
    ...
    if not summaries:
        return content # Return original content if all summaries fail
    ...
    if len(summaries) > 1:
        try:
            ...
            final_response = CLIENT.chat.completions.create(
                model="local-model", messages=[{"role": "user", "content": final_prompt}], temperature=0.2,
            )
            final_summary = final_response.choices[0].message.content
        except APIError as e:
            print(f"Warning: API error during final consolidation: {e}")
            final_summary = combined_summaries # Use combined if final pass fails
    else:
...
def process_html_file(file_path, path_map):
    ...
    try:
        ...
        enriched_content = summarize_content_in_batches(cleaned_markdown)
...
def main():
    """
    Main function to find, process in parallel, and save all wiki pages.
    """
